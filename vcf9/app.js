...
// ====== LocalStorage Keys ======
const LS_VERIFIED = 'ctw_verified_count';
const LS_TARGET = 'ctw_target_count';
const LS_END_DATE = 'ctw_end_date';
const LS_VCF_ENABLED = 'ctw_vcf_enabled';
const LS_VCF_NAME = 'ctw_vcf_name';
const LS_VCF_DATA = 'ctw_vcf_data';
...
function startCountdown() {
  function update() {
    let endDate = localStorage.getItem(LS_END_DATE);
    if (!endDate) {
      const d = new Date();
      d.setDate(d.getDate() + 30);
      endDate = d.toISOString();
      localStorage.setItem(LS_END_DATE, endDate);
    }
...
// ====== Verified Counts ======
function loadCounts() {
  const verified = parseInt(localStorage.getItem(LS_VERIFIED) || '0');
  const target = parseInt(localStorage.getItem(LS_TARGET) || '30');
  verifiedCountEl.textContent = verified;
  remainingCountEl.textContent = Math.max(0, target - verified);
}

// ====== VCF State ======
function loadVCFState() {
  const enabled = localStorage.getItem(LS_VCF_ENABLED) === 'true';
  const vcfName = localStorage.getItem(LS_VCF_NAME);
  const vcfData = localStorage.getItem(LS_VCF_DATA);
  if (enabled && vcfName && vcfData) {
    vcfSection.classList.remove('hidden');
    vcfDownloadBtn.textContent = `⬇️ Download ${vcfName}`;
    vcfDownloadBtn.href = vcfData;
    vcfDownloadBtn.download = vcfName;
  } else {
...
verifyForm.addEventListener('submit', (e) => {
  ...
  let verified = parseInt(localStorage.getItem(LS_VERIFIED) || '0');
  verified++;
  localStorage.setItem(LS_VERIFIED, verified.toString());
  loadCounts();
